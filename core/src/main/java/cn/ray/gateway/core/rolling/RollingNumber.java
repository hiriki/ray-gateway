package cn.ray.gateway.core.rolling;
        ...
        private class ListState {
            ...
            private ListState(AtomicReferenceArray<Bucket> data, int head, int tail) {
                ...
                if (head == 0 && tail == 0) {
                    ...
                    // 2. head = 0, tail = 1, dataLength = 7  size = 1
                    // 3. head = 0, tail = 2, dataLength = 7  size = 2
                    // ......
                    // 7. head = 1, tail = 0, dataLength = 7  size = 6
                    // 7. head = 2, tail = 1, dataLength = 7  size = 6
            ...
            private ListState incrementTail() {
                //	如果达到桶最大值 numBuckets = 6
                if (size == numBuckets) {
                    //	头索引+1, 尾索引+1
                    // 7. head = 0, tail = 6, size = 6, dataLength = 7  new(data, 1, 0);
                    // 8. head = 1, tail = 0, size = 6, dataLength = 7  new(data, 2, 1);
                    return new ListState(data, (head + 1) % dataLength, (tail + 1) % dataLength);
                } else {
                    //	头索引不变, 尾索引+1
                    // 1. head = 0, tail = 0, size = 0, dataLength = 7  new(data, 0, 1);
                    // 2. head = 0, tail = 1, size = 1, dataLength = 7  new(data, 0, 2);
                    // 3. head = 0, tail = 2, size = 2, dataLength = 7  new(data, 0, 3);
                    // ......
